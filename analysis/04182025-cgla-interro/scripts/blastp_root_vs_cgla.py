#!/usr/bin/env python3
"""
blastp_root_vs_cgla.py - Perform stringent BLASTp analysis between root.pep and C. glabrata proteome
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Compare root.pep vs C. glabrata proteome using BLASTp')
    parser.add_argument('--root_pep', required=True, help='Path to root.pep file')
    parser.add_argument('--cgla_ref', required=True, help='Path to C. glabrata reference proteome')
...
def run_bidirectional_blast(root_pep, cgla_ref, output_dir, evalue):
    """Run BLASTp in both directions and identify reciprocal best hits"""
    os.makedirs(output_dir, exist_ok=True)
    
    # Forward blast: root.pep → cgla_ref
    forward_output = os.path.join(output_dir, "root_to_cgla.tsv")
    run_blast(root_pep, cgla_ref, forward_output, evalue)
    
    # Reverse blast: cgla_ref → root.pep
    reverse_output = os.path.join(output_dir, "cgla_to_root.tsv")
    run_blast(cgla_ref, root_pep, reverse_output, evalue)
...
def analyze_results(forward_file, reverse_file, root_pep, cgla_ref, output_dir, min_identity, min_coverage):
    ...
    cgla_count = sum(1 for _ in SeqIO.parse(cgla_ref, "fasta"))
    ...
    reverse_match_rate = reverse_queries / cgla_count if cgla_count > 0 else 0
    rbh_count = len(rbh_df)
    rbh_rate = rbh_count / min(root_count, cgla_count) if min(root_count, cgla_count) > 0 else 0
    ...
    stats = {
        ...
        'total_cgla_sequences': cgla_count,
    ...
    forward_filtered.to_csv(os.path.join(output_dir, "root_to_cgla_filtered.csv"), index=False)
    reverse_filtered.to_csv(os.path.join(output_dir, "cgla_to_root_filtered.csv"), index=False)
    ...
    print(f"Total C. glabrata reference proteins: {cgla_count}")
    print(f"root.pep sequences with C. glabrata matches (≥{min_identity}% identity, ≥{min_coverage}% coverage): "
          f"{forward_queries} ({forward_match_rate*100:.1f}%)")
    print(f"C. glabrata proteins with root.pep matches (≥{min_identity}% identity, ≥{min_coverage}% coverage): "
    ...
    print("\nInterpretation:")
    if rbh_rate >= 0.9:
        print("✓ TRANSLATION QUALITY IS GOOD: Most C. glabrata proteins have high-quality matches in root.pep")
        print("  The orthology discrepancies are likely due to algorithmic differences between SYNERGY2 and YGOB/CGOB,")
        print("  not translation issues.")
    elif rbh_rate <= 0.5:
        print("✗ TRANSLATION QUALITY ISSUES DETECTED: Many C. glabrata proteins don't have good matches in root.pep")
        print("  The orthology discrepancies may be largely due to translation problems affecting SYNERGY2's ability")
        print("  to detect homology relationships.")
    else:
        print("⚠ PARTIAL TRANSLATION ISSUES: Some C. glabrata proteins have good matches in root.pep, but others don't")
        print("  The orthology discrepancies may be due to a combination of translation issues and algorithmic")
        print("  differences.")
...
def create_visualizations(forward_df, reverse_df, rbh_df, stats, output_dir):
    ...
    plt.title('Distribution of Percent Identity (root.pep → C. glabrata)')
    ...
    plt.title('Distribution of Query Coverage (root.pep → C. glabrata)')
    ...
    plt.title('Percent Identity vs. Query Coverage (root.pep → C. glabrata)')
    plt.xlabel('Percent Identity')
    ...
    if not rbh_df.empty:
        ...
        plt.xlabel('Forward Percent Identity (root.pep → C. glabrata)')
        plt.ylabel('Reverse Percent Identity (C. glabrata → root.pep)')
...
def main():
    ...
    forward_file, reverse_file = run_bidirectional_blast(
        args.root_pep, args.cgla_ref, args.output_dir, args.evalue
    )
    
    # Analyze results
    stats = analyze_results(
        forward_file, reverse_file, args.root_pep, args.cgla_ref, 
